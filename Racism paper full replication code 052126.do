...
cd "C:\Kaitlin\Racism Paper\International Sociology\Replication files\"

**Load clean data
use "C:\Kaitlin\Racism Paper\International Sociology\Replication files\racism_full data_clean.dta", clear
...
tab bcracist_restrict //862 endorsing racist beliefs
...
**********************************************************************************************************************
*Descriptive statistics and t test of continuous variables (perception, explanation, attitudes about inequality, age)
**********************************************************************************************************************
...
* Age requires preserve/restore (continuous)
preserve
    keep if country == 1
...
preserve
    ...
    anova_block reliattend cracialscale "country==1 & sample_us==1"
restore
...
preserve
    ...
    anova_block reliattend bracial_cat "country==2 & sample_nl==1"
restore
...
preserve
    ...
    anova_block reliattend cracialscale "country==2 & sample_nl==1"
restore
...
signtest mins = 10.7 if bracialscale == 2 & country == 1 & sample_us == 1  // p=0.0001 ***

* U.S. — cultural racism (non: 10.7, amb: 11.7, rac: 11.8)
forval k = 1/3 {
    sum mins if cracialscale == `k' & country == 1 & sample_us == 1, detail
}
signtest mins = 10.7 if cracialscale == 2 & country == 1 & sample_us == 1  // p=0.0207 *

* Netherlands — biological racism (non: 12.1, amb: 12.6, rac: 12.8)
forval k = 1/3 {
    sum mins if bracialscale == `k' & country == 2 & sample_nl == 1, detail
}
signtest mins = 12.1 if bracialscale == 2 & country == 2 & sample_nl == 1  // p=0.5266

* Netherlands — cultural racism (non: 11.2, amb: 12.6, rac: 13.3)
forval k = 1/3 {
    sum mins if cracialscale == `k' & country == 2 & sample_nl == 1, detail
}
signtest mins = 11.2 if cracialscale == 2 & country == 2 & sample_nl == 1  // p=0.0000 ***

//evidence from three out of four groups suggests statistically significantly different median survey completion time for non-racist and ambivalent racist, such that ambivalent racists take significantly longer
...
signtest mins = 11.23 if bracialscale == 2  // p=0.0002 ***

* Pooled (US + NL) — cultural racism (non: 10.95, amb: 12.08, rac: 12.32)
forval k = 1/3 {
    sum mins if cracialscale == `k', detail
}
signtest mins = 10.95 if cracialscale == 2  // p=0.0000 ***
...
***************************************************************************************************************
*****************************change figures to vertical rather than horizontal
